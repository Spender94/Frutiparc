class kaluga.Game extends kaluga.Slot{//}
	...
	*/
	...
	function initEndGame(timer){
		...
		// Only the actual "Challenge" mode (slot 0 in Menu.as → kaluga.game.Classic
		// → type "$classic") submits a score to the daily-challenge ranking.
		// Other sub-modes (Olympique, Chrono, Survie, Invasion, Piste, Sequence,
		// Préparation/$train) play locally without polluting the ranking.
			...
			if( this.level<3 && !list[this.level+1] ){
				var o = {
					...
					list:[
						{
							type:"congrat",
							text:"Vous avez d�bloqu� le mode "+this.mng.difNameList[this.level+1]+"!!\n",
							id:10
						}
					]
				};
				...
				case 2:
					...
					var o = {
						...
						list:[
							{
								type:"congrat",
								text:"Vous avez d�bloqu� une nouvelle tzongre : "+this.mng.tzInfo[id].name+"!!\n",
								id:id
							}
						]
					};
	...
	function addKagulga(){
		...
		var o = {
			...
			list:[
				{
					type:"congrat",
					text:"Vous avez gagn� la kagulga !\nEn v�ritable cuir de Tzongre, cette cagoule vous permettra de garder la fruticlasse en toutes circonstances !",
					id:14
				}
			]
		};
